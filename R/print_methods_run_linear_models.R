...
print.run_model_result <- function(x, ...) {
  ...
  message("Note: Since you didn’t assign the result to a variable, components like residuals won't be accessible later.")
  print(x$tidy)
  invisible(x)
}
...
print.run_model_result_list <- function(x, ...) {
  ...
  message("Note: Since you didn’t assign the result to a variable, these components won’t be accessible later.")
  print(names(x))
  invisible(x)
}
